...
class _MemberDetailScreenState extends State<MemberDetailScreen> {
  ...
  Future<void> fetchData() async {
    ...
    final feesSnapshot = await firestore.collection('gyms').doc(widget.gymId)
        .collection('members').doc(widget.uid).collection('fees').orderBy('dueDate', descending: true).get();

    setState(() {
      name = userDoc.data()?['name'] ?? 'Unknown';
      contactNumber = userDoc.data()?['contactNumber'] ?? '--';
      status = memberDoc.data()?['status'] ?? 'Pending';
      plan = memberDoc.data()?['membershipPlan'] ?? '--';
      joinedAt = (memberDoc.data()?['createdAt'] as Timestamp?)?.toDate();
      validUntil = (memberDoc.data()?['validUntil'] as Timestamp?)?.toDate();
      totalFees = memberDoc.data()?['totalFeesPaid'] ?? 0;
  ...
  @override
  Widget build(BuildContext context) {
    if (loading) return const Scaffold(backgroundColor: Colors.black, body: Center(child: CircularProgressIndicator(color: Colors.yellowAccent)));

    return Scaffold(
      backgroundColor: Colors.black,
      appBar: AppBar(backgroundColor: Colors.black, title: const Text("MEMBER PROFILE", style: TextStyle(fontSize: 16, color: Colors.white))),
      body: SingleChildScrollView(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          children: [
            ...
            _buildActionRow(),
            const SizedBox(height: 30),
            _buildNavigationTile("Attendance History", Icons.calendar_month, () {
               Navigator.push(context, MaterialPageRoute(builder: (_) => AttendanceScreen(uid: widget.uid, gymId: widget.gymId)));
            }),
            _buildNavigationTile("Payment History", Icons.history, () {
               Navigator.push(context, MaterialPageRoute(builder: (_) => PaymentHistoryScreen(uid: widget.uid, gymId: widget.gymId)));
            }),
            const SizedBox(height: 30),
            _sectionHeader("FEES & SUBSCRIPTION"),
            _infoTile("Plan Type", plan),
            _infoTile("Valid Until", validUntil != null ? DateFormat('dd MMM yyyy').format(validUntil!) : "--"),
            const SizedBox(height: 20),
            ...fees.take(2).map((f) => _feeTile(f['plan'], f['amount'], f['paid'])),
          ],
        ),
      ),
    );
    ...
    return Column(
      children: [
        CircleAvatar(
          radius: 50,
          backgroundColor: Colors.yellowAccent.withOpacity(0.1),
          child: Text(name[0].toUpperCase(), style: const TextStyle(fontSize: 40, color: Colors.yellowAccent, fontWeight: FontWeight.bold)),
        ),
        const SizedBox(height: 15),
        Text(name, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
        Text(status.toUpperCase(), style: TextStyle(color: status == 'active' ? Colors.greenAccent : Colors.redAccent, fontSize: 12, letterSpacing: 1.2)),
      ],
    );
  }
  ...
  Widget _buildActionRow() {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
      children: [
        _actionCircle(Icons.phone, "Call", Colors.blue, () => _makeCall()),
        _actionCircle(Icons.message, "WhatsApp", Colors.green, () => _sendWhatsApp()),
      ],
    );
  }
  ...
  Widget _feeTile(String plan, num amount, bool paid) {
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(color: paid ? Colors.green.withOpacity(0.05) : Colors.red.withOpacity(0.05), borderRadius: BorderRadius.circular(10)),
      child: Row(
        children: [
          Text(plan, style: const TextStyle(color: Colors.white)),
          const Spacer(),
          Text("Rs $amount", style: TextStyle(color: paid ? Colors.greenAccent : Colors.redAccent)),
        ],
      ),
    );
  }

  Widget _sectionHeader(String title) => Align(alignment: Alignment.centerLeft, child: Text(title, style: const TextStyle(color: Colors.yellowAccent, fontSize: 12, fontWeight: FontWeight.bold)));

  void _makeCall() async => await launchUrl(Uri.parse("tel:$contactNumber"));
  void _sendWhatsApp() async => await launchUrl(Uri.parse("https://wa.me/${contactNumber.replaceAll(RegExp(r'[^0-9]'), '')}"));
}